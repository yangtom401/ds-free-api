...
impl DsClient {
    ...
    pub async fn get_wasm(&self) -> Result<Bytes, ClientError> {
        if !self.wasm_url.starts_with("http://") && !self.wasm_url.starts_with("https://") {
            let path = self.wasm_url.trim_start_matches("file://");
            let data = std::fs::read(path).map_err(|e| ClientError::Status {
                status: 404,
                body: format!("读取本地 WASM 失败 ({path}): {e}"),
            })?;
            return Ok(Bytes::from(data));
        }

        // 尝试下载 WASM
        match self.http.get(&self.wasm_url).send().await {
            Ok(resp) => {
                let status = resp.status();
                if status.is_success() {
            ...
            Err(e) => {
                warn!(target: "ds_core::client", "下载 WASM 失败 ({}), 尝试读取本地缓存: {}", self.wasm_url, e);
            }
        }

        // 下载失败，尝试从本地加载（提取 URL 文件名，如 sha3_wasm_bg.7b9ca65ddd.wasm）
        let filename = self.wasm_url.split('/').last().unwrap_or("sha3_wasm_bg.7b9ca65ddd.wasm");
        if let Ok(data) = std::fs::read(filename) {
            warn!(target: "ds_core::client", "已成功从本地加载回退 WASM 文件: {}", filename);
            return Ok(Bytes::from(data));
        }
