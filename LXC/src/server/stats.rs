...
impl Stats {
    ...
    pub fn persist_now(&self) {
        if let Some(st) = self.prepare_stats_store() {
    ...
    /// 异步持久化并等待完成（用于优雅关闭）
    pub async fn persist_now_async(&self) {
        if let Some(st) = self.prepare_stats_store() {
            if let Some(ref store) = self.store {
                if let Err(e) = store.save_stats(&st).await {
                    log::warn!(target: "stats", "持久化失败: {}", e);
    ...
    /// 准备要持久化的 StatsStore 数据
    fn prepare_stats_store(&self) -> Option<super::store::StatsStore> {
