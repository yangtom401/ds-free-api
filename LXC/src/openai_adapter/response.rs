...
/// 尝试直接将 tool_text 解析为工具调用数组，成功则跳过 repair 模型
///
/// 处理两种格式：
/// 1. JSON 数组 `[{"name":"x", "arguments":{...}}]`
/// 2. 单个 JSON 对象 `{"name":"x", "arguments":{...}}`（自动包装为数组）
fn try_parse_tool_calls_directly(text: &str) -> Option<Vec<ToolCall>> {
    let trimmed = text.trim();
    // 定位 JSON 数组或对象
    let json_str: String = if let Some(start) = trimmed.find('[') {
                ...
                RepairState::Forwarding => {
                    match this.inner.as_mut().as_pin_mut().map(|p| p.poll_next(cx)) {
                        ...
                        Some(Poll::Ready(Some(Err(OpenAIAdapterError::ToolCallRepairNeeded(
                            ...
                            let snippet_end = tool_text.char_indices().nth(200).map(|(i, _)| i).unwrap_or(tool_text.len());
                            let snippet = &tool_text[..snippet_end];
                            warn!(
                                target: "adapter",
                                "RepairStream 捕获修复请求: len={}, snippet='{}'",
                                tool_text.len(),
                                snippet
                            );
                            trace!(target: "adapter", ">>> repair: accepting tool_text len={}, snippet='{}'", tool_text.len(), snippet);
                            drop(this.inner.as_mut().get_mut().take());
                            // 优先尝试直接解析为 JSON 工具调用，避免调用 repair 模型
                            if let Some(calls) = try_parse_tool_calls_directly(&tool_text) {
                                info!(
                                    target: "adapter",
                                    "tool_calls 直接解析成功: {} 个调用，跳过 repair",
                                    calls.len()
                                );
