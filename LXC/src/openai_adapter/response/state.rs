                ...
                if let Some(s) = val.as_str() {
                    ...
                    if s == "FINISHED" || s == "INCOMPLETE" {
                        ...
                        if !has_response {
                            warn!(
                                target: "adapter",
                                "状态机 {} 但无 RESPONSE 内容: fragments={:?}, accumulated_token_usage={:?}，注入占位符",
                                s,
                                self.fragments.iter().map(|f| format!("{}/{}", f.ty, f.content.len())).collect::<Vec<_>>(),
                                self.accumulated_token_usage
                            );
                            // 模型只有思考过程、无实际回答时，注入一个最小占位文本，
                            // 防止客户端（如 Aider）因收到空响应而视为"连接中断"
                            frames.push(DsFrame::ContentDelta(
                                "\u{200b}".to_string() // 零宽空格，不影响渲染但满足协议非空要求
                            ));
                        }
                    }
                        ...
                        if let Some(ty) = item.get("type").and_then(|t| t.as_str()) {
                            ...
                            if !content.is_empty() {
                                match ty {
                                    FRAG_THINK => frames.push(DsFrame::ThinkDelta(content)),
                                    FRAG_RESPONSE => {
                                        let cleaned = content.replace("FINISHED", "")
                                            .replace("SEARCHING", "").replace("WEB_SEARCH", "")
                                            .replace("SEARCH", "").trim_start().to_string();
                                        if !cleaned.is_empty() {
                                            frames.push(DsFrame::ContentDelta(cleaned));
                                        }
                                    }
                                    "TIP" => frames.push(DsFrame::ContentDelta(format!("\n\n[DeepSeek System TIP: {}]\n", content))),
                                    _ => {}
                                }
                            }
                        }
