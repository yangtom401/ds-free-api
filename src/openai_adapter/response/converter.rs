        ...
        loop {
            ...
            match this.inner.as_mut().poll_next(cx) {
                Poll::Ready(Some(Ok(frame))) => match frame {
                    ...
                    {
                        trace!(target: "adapter", ">>> conv: finish={}", status);
                        *this.finished = true;
                        // 将 usage 合并到 finish chunk，确保下游（如 Anthropic）能拿到 completion_tokens
                        let reason = if status == "INCOMPLETE" { "length" } else { "stop" };
                        let mut chunk = make_chunk(this.model, Delta::default(), Some(reason));
                ...
                Poll::Ready(None) => {
                    *this.inner_done = true;
                    if !*this.finished {
                        ...
                        *this.finished = true;
                        // 如果被强行掐断，作为长度截断抛出
                        let mut chunk = make_chunk(this.model, Delta::default(), Some("length"));
                        if *this.include_usage
                            && let Some(u) = this.usage_value.take()
                        {
                            chunk.usage = Some(make_usage(*this.prompt_tokens, u));
