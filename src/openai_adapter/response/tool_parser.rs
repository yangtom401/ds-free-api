...
pub fn parse_tool_calls_with(xml: &str, cfg: &TagConfig) -> Option<(Vec<ToolCall>, String)> {
    ...
    let inner_raw = &xml[after_start..inner_end];

    // Bug #12: 模型幻觉有时输出双重开始标签，如：
    //   <|begin|>
    //   <|begin|>[{...}]
    // inner 里会包含一个多余的 start tag，需要跳过它，直接定位到 JSON 内容
    let inner = if let Some((nested_pos, nested_tag)) = find_start_tag_with(inner_raw, cfg) {
        let after_nested = nested_pos + nested_tag.len();
        // 只有在多余标签后面有实际内容时才剥离，否则原样保留
        let candidate = &inner_raw[after_nested..];
        ...
        loop {
            ...
            match this.inner.as_mut().poll_next(cx) {
                        ...
                        match &mut this.state {
                            ToolParseState::Detecting { buffer } => {
                                ...
                                if let Some((pos, start_tag)) = maybe_tag {
                                    ...
                                    if let Some((end_pos, matched_end)) = find_end_tag_with(
                                        ...
                                        if inner_content.is_empty() || inner_content == "[]" || inner_content == "{}" {
                                            debug!(target: "adapter", "tool_parser 检测到空的工具调用块，忽略此标签块");
                                            let tail = rest[end_abs..].to_string();
                                            *this.state = ToolParseState::Detecting { buffer: tail };
                                            choice.delta.content = if before.is_empty() { None } else { Some(before) };
                                            return Poll::Ready(Some(Ok(chunk)));
                                        }
                                        ...
                                        if let Some((calls, _)) = parse_tool_calls(collected) {
                                            ...
                                            *this.state = ToolParseState::Done;
                                            ...
                                            {
                                                let safe_end = floor_char_boundary(collected, 500);
                            ...
                            ToolParseState::CollectingXml { buf, start_tag } => {
                                ...
                                if let Some((end_pos, en_tag)) = find_end_tag_with(
                                    ...
                                    // 先把需要的内容以 owned String 提取，避免后续 split_off 的可变借用冲突
                                    let inner_content = buf[start_end..end_pos].trim().to_string();
                    ...
                    match &mut this.state {
                        ...
                        ToolParseState::CollectingXml { buf, start_tag: _ } => {
                            if choice.finish_reason.is_some() {
                                ...
                                if let Some((pos, _tag)) = find_start_tag_with(&flushed, this.tag_config) {
                                    let after_start = pos + _tag.len();
                                    let inner_content = flushed[after_start..].trim();
                                    if inner_content.is_empty() || inner_content == "[]" || inner_content == "{}" {
                                        is_empty_block = true;
                                    }
                                }
                                if is_empty_block {
                                    debug!(target: "adapter", "tool_parser finish 时检测到空的工具调用块，忽略");
                                    choice.delta.content = None;
                                    *this.state = ToolParseState::Done;
                                    return Poll::Ready(Some(Ok(chunk)));
                                }
                ...
                Poll::Ready(None) => match std::mem::replace(this.state, ToolParseState::Done) {
                    ...
                    ToolParseState::CollectingXml { buf, start_tag: _ } => {
                        let mut is_empty_block = false;
                        if let Some((pos, _tag)) = find_start_tag_with(&buf, this.tag_config) {
                            let after_start = pos + _tag.len();
                            let inner_content = buf[after_start..].trim();
                            if inner_content.is_empty() || inner_content == "[]" || inner_content == "{}" {
                                is_empty_block = true;
                            }
                        }
                        if is_empty_block {
                            debug!(target: "adapter", "tool_parser 流结束时检测到空的工具调用块，忽略");
                            let chunk = make_end_chunk(this.model, Delta::default(), "stop");
                            return Poll::Ready(Some(Ok(chunk)));
                        }
