...
/// POST /admin/api/relogin — 重新查验指定账号（重新登录）
pub(crate) async fn admin_relogin(
    State(state): State<AppState>,
    body: axum::body::Bytes,
) -> Response {
    let req: ReloginRequest = match serde_json::from_slice(&body) {
        Ok(r) => r,
        Err(e) => return error_response(StatusCode::BAD_REQUEST, &format!("请求格式错误: {}", e)),
    };

    match state.adapter.re_login_single(&req.email).await {
        Ok(_) => json_response(&serde_json::json!({"ok": true})),
        Err(e) => error_response(StatusCode::BAD_REQUEST, &e),
    }
}

/// POST /admin/api/check_account — 查验并添加指定账号（重新登录/初始化）
pub(crate) async fn admin_check_account(
    State(state): State<AppState>,
    body: axum::body::Bytes,
) -> Response {
    let req: CheckAccountRequest = match serde_json::from_slice(&body) {
        Ok(r) => r,
        Err(e) => return error_response(StatusCode::BAD_REQUEST, &format!("请求格式错误: {}", e)),
    };
