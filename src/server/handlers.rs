//! HTTP 路由处理 —— 薄路由层，委托给 OpenAIAdapter / AnthropicCompat
//!
...
pub(crate) async fn chat_completions(
    ...
    log::debug!(target: "http::request", "req={} POST /v1/chat/completions model={} stream={}", request_id, req.model, req.stream);
    ...
    match result.data {
        ChatOutput::Stream(stream) => {
            ...
            log::debug!(target: "http::response", "req={} 200 SSE stream started, model={}", request_id, model);
        ...
        ChatOutput::Json(json) => {
            ...
            log::debug!(target: "http::response", "req={} 200 JSON response {} bytes, model={}", request_id, bytes.len(), model);
            Ok(Response::builder()
...
pub(crate) async fn anthropic_messages(
    ...
    log::debug!(target: "http::request", "req={} POST /anthropic/v1/messages model={} stream={}", request_id, req.model, req.stream);
    let model = req.model.clone();
    ...
    match result.data {
        AnthropicOutput::Stream(stream) => {
            ...
            log::debug!(target: "http::response", "req={} 200 Anthropic SSE stream started, model={}", request_id, model);
            Ok(SseBody::new(guarded)
                .with_header(X_DS_ACCOUNT, &mask_account_id(&result.account_id))
                .into_response())
        }
        AnthropicOutput::Json(json) => {
            ...
            let bytes = serde_json::to_vec(&json).unwrap();
            log::debug!(target: "http::response", "req={} 200 Anthropic JSON response {} bytes, model={}", request_id, bytes.len(), model);
            Ok(Response::builder()
                .status(StatusCode::OK)
                .header(header::CONTENT_TYPE, "application/json")
                .header(X_DS_ACCOUNT, &mask_account_id(&result.account_id))
